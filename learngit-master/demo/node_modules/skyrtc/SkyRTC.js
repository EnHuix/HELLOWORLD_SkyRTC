...
function SkyRTC() {
	...
	this.on('__join', function(data, socket) {
		...
		for (i = 0, m = curRoom.length; i < m; i++) {
			curSocket = curRoom[i];
			// curSocket.user.socketId=curSocket.id;
			users.push(curSocket.user);
			// socket_ids.push(curSocket.id);
		}
	...
	this.on('_chat',function(data){
		console.log(data);
		var dst = this.getSocket(data.dst_socketId);
		if (dst) {
			dst.send(JSON.stringify({
				"eventName": "_new_message",
				"data": {
					"src_user":data.socketUser,
					"src_socketId":data.socketId,
